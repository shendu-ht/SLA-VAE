...
from src.common.constant import KPIADParams
...
def load_data(data_path: str = None):
    """load training data for single kpi anomaly detection"""

    if data_path is None:
        data_path = os.path.join(KPIADParams.ABSOLUTE_PATH, KPIADParams.SAMPLE_PATH)
...
class KpiAnomalyDetection:
    """Anomaly detection model for single KPI anomaly detection."""

    def __init__(self, in_dim: List[int], latent_dim: int = None, hidden: List[int] = None, **kwargs):

        if latent_dim is None:
            latent_dim = KPIADParams.LATENT_LEN
        if hidden is None:
            hidden = KPIADParams.HIDDEN_LAYERS
    ...
    def dumps_model(self, path: str = None):
        ...
        if path is None:
            path = os.path.join(KPIADParams.ABSOLUTE_PATH, KPIADParams.PARAM_PATH)
...
def kpi_ad_api(dataframe: pd.DataFrame, model_path, **kwargs):
    """
    ...
    try:
        ...
        features = tsf_ext.kpi_feature_ext(series_verify(df['value']), target_idx)

        kpi_ad = KpiAnomalyDetection(list(features.shape[1:]), 1, **kwargs)
